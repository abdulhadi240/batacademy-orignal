...
const Card = ({ number, title, description , slug , param }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-6 rounded-lg hover:shadow-md bg-white group">
      <div
        className={cn(
      ...
      <Link href={`/${param}/consulting_services/${slug}`} className="px-4 py-2 text-xs text-black transition border-2 rounded group-hover:bg-primary group-hover:text-white border-primary hover:bg-blue-700">
        More Details
      </Link>
    </div>
  );
